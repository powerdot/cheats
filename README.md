...
* * [Функции](https://github.com/powerdot/cheats/blob/master/functions.md)
* * [Четное & нечетное](#четное--нечетное)
* * [Смена типа данных](#смена-типа-данных)
* * [Работа со строчками](#работа-со-строчками)
...
* * [Примеры с function](https://github.com/powerdot/cheats/blob/master/functions.md)
...
**Ты знаешь**, существуют разные типы данных:
- Вообще все числа, без жесткой привязки, без точного типа данных - 5, 2.332: Num, Number
- Целые числа - 5, 6: Int, Integer
- Числа с плавающей точкой - 2.45, 0.3233 Float
- Строчки - "5", "Привет": Str, String
- Объекты - {a:1, b:2}: Obj, Object
- Массивы - [1,2,3]: Array
- Функции - console.log(): Function

Тип можно узнать очень просто, через **typeof**:
```js
var 
...
// Однако массив, по его мнению тоже Object!
console.log(typeof []); // "object"
// Поэтому, у массивов другой способ:
Array.isArray( [] ); // true
Array.isArray( {} ); // false
```

Так как же менять типы данных?
```js
var a = 5;
// Хочу из интегера в стринг (integer -> string, число -> текст)
var new_a = a.toString(); // Теперь это "5".

// Обратно (string -> integer):
var new_new_a = parseInt( new_a );

// теперь флоат в интегер:
var b = 5.33;
var new_b = parseInt(b); // 5

// теперь флоат в стринг:
var new_new_b = b.toString(); // "5.33"

// теперь стринг во флоат:
var new_new_new_b = parseFloat( new_new_b ); // 5.33
```

### Работа со строчками

Строчки можно резать, превращать в массивы, искать текст, вертеть на хуе - что угодно!  
Важно понять, что **СТРОКА - это МАССИВ СИМВОЛОВ** ШОК!!!  
...
```js
// Получаем первые 6 букв из текста
var str = "Привет, как дела?";
console.log( str.slice(0, 6) ); // "Привет"

// Получаем последние 6 букв из текста
var str = "Привет, как дела?";
console.log( str.slice(-6) ); // " дела?"

// Режем строку на массив по пробелу:
var str = "Привет, как дела?";
var array = str.split(" "); // Режем по пробелу
console.log( array ); // ["Привет,", "как", "дела?"]

// Превращаем все в капс:
var str = "Привет, как дела?";
var str_up = str.toUpperCase();
console.log( str_up ); // "ПРИВЕТ, КАК ДЕЛА?"

// Превращаем все в маленькие буквы:
var str = "Привет, как дела?";
var str_low = str.toLowerCase();
console.log( str_low ); // "привет, как дела?"

// Ищем: есть ли такой текст в тексте
var str = "Привет, как дела?";
console.log( str.indexOf('как') ); // 8 - возвращает индекс, если есть совпадение
console.log( str.indexOf('чпек') ); // -1 - возвращает такой индекс, если ничего не нашел
console.log( str.indexOf('Прив') ); // 0 - Есть и стоит на 0 индексе
```

Как ты заметел, мы можем записывать строки любыми кавычками:
```js
"Привет"
'Привет'
```
Но есть и еще одни крутые кавычки:
```js
`Привет` // вот такие крутые! Они очень особенные

// Смотри:
var name = "илья";
console.log( `привет, ${name}, как дела?` ); // привет, илья, как твои дела

// это очень упрощает написание строк, их не нужно миллиард раз складывать
// как здесь :/
console.log( "привет" + name + ", как дела?" );
```

Они называются ***Гравис***
